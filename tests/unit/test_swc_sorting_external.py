...
def test_resample_external_reads_env(tmp_path: Path, monkeypatch) -> None:
    calls: list[list[str]] = []
    ...
    swc_out = tmp_path / "out.swc"
    swc_in.write_text("1 1 0 0 0 1 -1\n", encoding="utf-8")
    ext.resample_swc_external(swc_in, swc_out, step=2.0)
    assert calls[0][0] == "vaa3d-x"


def test_resample_external_version_override(tmp_path: Path, monkeypatch) -> None:
    calls: list[list[str]] = []
    ...
    swc_out = tmp_path / "out.swc"
    swc_in.write_text("1 1 0 0 0 1 -1\n", encoding="utf-8")
    ext.resample_swc_external(swc_in, swc_out, step=2.0, vaa3d_version="3")
    assert calls[0][0] == "vaa3d-3"
